  ...
  return (
    <div>
      ...
      <div style={{minHeight:"90vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"5rem 2rem", position:"relative", overflow:"hidden"}}>
        ...
        <p style={{fontSize:"17px", color:MID, maxWidth:"500px", margin:"0 auto 3rem", lineHeight:1.75}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))", gap:"1px", background:BORDER, border:`1px solid ${BORDER}`, borderRadius:"4px", overflow:"hidden"}}>
          {whyPoints.map((p, i) => (
            <div key={i} style={{background:BG, padding:"1.75rem 2rem"}}>
              <div style={{fontSize:"13px", fontWeight:600, color:FG, marginBottom:"6px"}}>{p.title}</div>
              <div style={{fontSize:"13px", color:MID, lineHeight:1.65}}>{p.body}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Social proof strip */}
      <div style={{borderTop:`1px solid ${BORDER}`, borderBottom:`1px solid ${BORDER}`, padding:"3rem 2rem", textAlign:"center"}}>
        <p style={{fontSize:"11px", letterSpacing:"0.14em", color:MID, textTransform:"uppercase", fontWeight:500, marginBottom:"2rem"}}>How we stack up</p>
        <div style={{display:"flex", flexWrap:"wrap", justifyContent:"center", gap:"3rem", maxWidth:"900px", margin:"0 auto"}}>
          {[
            { stat:"$79/mo", label:"Up to 10 barbers — flat" },
            { stat:"$0", label:"Hidden fees to your clients" },
            { stat:"Every tier", label:"SMS & email included" },
            { stat:"Day one", label:"AI tools, no upgrade required" },
          ].map((s, i) => (
            <div key={i} style={{textAlign:"center"}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif", fontSize:"42px", color:G, letterSpacing:"0.02em"}}>{s.stat}</div>
              <div style={{fontSize:"12px", color:MID, marginTop:"4px"}}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{padding:"6rem 2rem", textAlign:"center"}}>
        ...
        <p style={{fontSize:"15px", color:MID, marginBottom:"2.5rem"}}>Month 1 at your chosen rate. Month 2 on us.</p>
...
function PricingTab({ setTab }) {
  ...
  return (
    <div style={{maxWidth:"1100px", margin:"0 auto", padding:"4rem 2rem"}}>
      ...
      <p style={{fontSize:"15px", color:MID, maxWidth:"480px", marginBottom:"3rem", lineHeight:1.7}}>Every tier includes SMS and email at no extra cost. No per-barber fees. No hidden charges to your clients. Month 2 free when you sign up today.</p>
        ...
        {tiers.map(t => (
          <div key={t.id} style={{background:t.highlight?"#141414":BG, border:t.highlight?`1px solid ${G}`:t.enterprise?`1px solid ${GOLD}`:`1px solid ${BORDER}`, borderRadius:"4px", padding:"1.75rem 1.5rem", display:"flex", flexDirection:"column", position:"relative"}}>
            ...
            <div style={{marginBottom:"0.25rem"}}>
              ...
              <span style={{fontSize:"13px", color:MID, marginLeft:"6px"}}>{t.per}</span>
            </div>
            <div style={{fontSize:"12px", color:MID, marginBottom:"1rem"}}>{t.sub}</div>
            <p style={{fontSize:"13px", color:"#9CA3AF", lineHeight:1.6, flex:1, marginBottom:"1.5rem"}}>{t.desc}</p>
      ...
      <div style={{textAlign:"center", marginBottom:"2rem"}}>
        <button onClick={() => setShowTable(v=>!v)} style={{background:"transparent", border:`1px solid #2D2D2D`, color:MID, fontSize:"13px", padding:"10px 24px", borderRadius:"2px", cursor:"pointer", letterSpacing:"0.05em"}}>
                ...
                <Fragment key={section.category}>
                  ...
                  {section.rows.map(row => (
                    <tr key={row.label} style={{borderBottom:`1px solid #141414`}}>
                      <td style={{padding:"9px 12px", color:"#9CA3AF"}}>{row.label}</td>
      ...
      <div style={{borderTop:`1px solid ${BORDER}`, marginTop:"4rem", padding:"4rem 0", textAlign:"center"}}>
        ...
        <p style={{fontSize:"15px", color:MID, maxWidth:"400px", margin:"0 auto 2rem", lineHeight:1.7}}>Custom plan, white-labeling, dedicated support, and pricing that scales with you.</p>
  ...
  const planOptions = [
    {
      ...
      pros:["Online booking page","Guest booking — no account required","SMS & email notifications included","AI assistant chat bubble","Client list & CSV export","Cash tracker & quick checkout","Google / Apple Calendar sync"],
    ...
    },
    {
      ...
      pros:["Up to 10 barbers — one flat price","Role-based access (owner / manager / barber)","Barber invite flow","Commission tracking","AI client import (CSV / Excel / PDF)","Schedule optimizer","Full reporting & analytics","Payroll available as add-on"],
    ...
    },
    {
      ...
      pros:["Up to 12 barbers per location","Up to 5 locations","Custom branding per location","Centralized multi-location dashboard","Everything in Pro included","Best value for growing chains"],
  ...
  const PricingModal = () => (
    <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"2rem 1rem"}}>
      <div style={{background:"#0D0D0D",border:`1px solid ${BORDER}`,borderRadius:"6px",width:"100%",maxWidth:"1020px",padding:"2.5rem",position:"relative"}}>
        <button onClick={()=>setPricingModalOpen(false)} style={{position:"absolute",top:"1.25rem",right:"1.25rem",background:"transparent",border:`1px solid ${BORDER}`,color:MID,fontSize:"13px",padding:"6px 14px",borderRadius:"2px",cursor:"pointer",fontFamily:"inherit"}}>Close ✕</button>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"10px",marginBottom:"2.5rem"}}>
          {[
            ...
            <div key={t.name} style={{background:t.h?"#161616":BG,border:`1px solid ${t.h?G:t.c===GOLD?GOLD:BORDER}`,borderRadius:"4px",padding:"1.25rem",position:"relative"}}>
              ...
              <div style={{fontSize:"11px",color:MID,marginTop:"2px"}}>{t.sub}</div>
        ...
        <p style={{fontSize:"11px",letterSpacing:"0.12em",color:MID,textTransform:"uppercase",fontWeight:500,marginBottom:"1rem"}}>How we compare to the competition</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(175px,1fr))",gap:"10px"}}>
          {[
            ...
            <div key={c.name} style={{background:"#111",border:`1px solid ${BORDER}`,borderRadius:"4px",padding:"0.9rem 1rem"}}>
              ...
              <div style={{fontSize:"11px",color:MID,marginBottom:"8px"}}>{c.price}</div>
              <div style={{borderTop:`1px solid ${BORDER}`,paddingTop:"6px"}}>
                {c.cons.map((d,i)=>(
                  <div key={i} style={{display:"flex",gap:"6px",marginBottom:"3px"}}>
                    ...
                    <span style={{fontSize:"11px",color:"#6B7280",lineHeight:1.4}}>{d}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  if (stage === "account") return (
    <div style={{minHeight:"80vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"4rem 2rem"}}>
      <div style={{width:"100%", maxWidth:"420px"}}>
        ...
        <p style={{fontSize:"14px", color:MID, marginBottom:"2rem", lineHeight:1.65}}>Almost there. Set your login credentials and we'll send a verification code to confirm your email.</p>
        <div style={{display:"flex", flexDirection:"column", gap:"12px"}}>
          <div>
            <label style={{fontSize:"11px", letterSpacing:"0.1em", color:MID, textTransform:"uppercase", fontWeight:500, display:"block", marginBottom:"6px"}}>Email address</label>
          ...
          <div>
            <label style={{fontSize:"11px", letterSpacing:"0.1em", color:MID, textTransform:"uppercase", fontWeight:500, display:"block", marginBottom:"6px"}}>Choose a password</label>
            <input type="password" value={accountPassword} onChange={e=>setAccountPassword(e.target.value)} placeholder="At least 8 characters"
              style={{width:"100%", padding:"13px 16px", background:"#111", border:`1px solid ${BORDER}`, color:FG, fontSize:"14px", borderRadius:"4px", fontFamily:"inherit", outline:"none", boxSizing:"border-box"}} />
          </div>
          <div>
            <label style={{fontSize:"11px", letterSpacing:"0.1em", color:MID, textTransform:"uppercase", fontWeight:500, display:"block", marginBottom:"6px"}}>Confirm password</label>
        ...
        <p style={{fontSize:"12px", color:MID, marginTop:"1.5rem", lineHeight:1.6}}>
  ...
  if (stage === "verify") return (
    <div style={{minHeight:"80vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"4rem 2rem"}}>
      <div style={{width:"100%", maxWidth:"420px", textAlign:"center"}}>
        ...
        <p style={{fontSize:"14px", color:MID, maxWidth:"340px", margin:"0 auto 2rem", lineHeight:1.7}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:"12px", alignItems:"center"}}>
          ...
          <button onClick={()=>{}} style={{background:"none", border:"none", fontSize:"12px", color:MID, cursor:"pointer", fontFamily:"inherit", marginTop:"4px"}}>
            Resend code
          </button>
        </div>
      </div>
    ...
    return (
      <div style={{padding:"4rem 2rem", maxWidth:"860px", margin:"0 auto"}}>
        ...
        <div style={{textAlign:"center", marginBottom:"4rem"}}>
          ...
          <p style={{fontSize:"16px", color:MID, maxWidth:"460px", margin:"0 auto 0.5rem", lineHeight:1.75}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:"2px"}}>
          {steps.map((s, i) => (
            <div key={i} style={{display:"flex", gap:"1.5rem", alignItems:"flex-start", padding:"1.25rem 1.5rem", background: i % 2 === 0 ? "#0F0F0F" : "#111", border:`1px solid ${BORDER}`, borderRadius: i===0?"4px 4px 0 0" : i===steps.length-1?"0 0 4px 4px":"0"}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:"14px", fontWeight:600, color:FG, marginBottom:"4px"}}>{s.title}</div>
                <div style={{fontSize:"13px", color:MID, lineHeight:1.65}}>{s.body}</div>
              </div>
        ...
        <div style={{textAlign:"center", marginTop:"3rem"}}>
          ...
          <p style={{fontSize:"12px", color:MID, marginTop:"1rem"}}>You can always come back to this guide under Help → Getting Started.</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{minHeight:"80vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"4rem 2rem", position:"relative"}}>
      ...
      <div style={{width:"100%", maxWidth:"620px"}}>
        ...
        {q.type === "terms" ? (
          <div>
            {/* Scrollable T&C box */}
            <div style={{background:"#0A0A0A", border:`1px solid ${BORDER}`, borderRadius:"4px", padding:"1.5rem", maxHeight:"340px", overflowY:"auto", marginBottom:"1.5rem", fontSize:"12px", color:"#6B7280", lineHeight:1.75}}>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>1. Acceptance of Terms</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>2. Description of Service</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>3. Subscription & Billing</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>4. Add-On Services</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>5. SMS & Email Communications Consent</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>6. Data & Privacy</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>7. Client Data & Imported Credentials</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>8. Acceptable Use</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>9. Limitation of Liability</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>10. Termination</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>11. Governing Law</p>
              ...
              <p style={{color:"#9CA3AF", fontWeight:500, marginBottom:"4px"}}>12. Changes to Terms</p>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:"12px", marginBottom:"2rem"}}>
              {[
                ...
                <label key={c.key} style={{display:"flex", gap:"12px", alignItems:"flex-start", cursor:"pointer"}}>
                  ...
                  <span style={{fontSize:"12px", color:MID, lineHeight:1.6}}>{c.label}</span>
                </label>
              ))}
            </div>
            ...
            <div style={{display:"flex", gap:"10px", alignItems:"center"}}>
              {step > 0 && <button onClick={handleBack} style={{background:"transparent", border:`1px solid ${BORDER}`, color:MID, padding:"11px 20px", fontSize:"13px", borderRadius:"2px", cursor:"pointer", fontFamily:"inherit"}}>← Back</button>}
          ...
          <div>
                  ...
                  <div style={{padding:"1.25rem 1.5rem"}}>
                    <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"0.75rem"}}>
                      <div>
                        ...
                        <span style={{fontSize:"12px", color:MID, marginLeft:"10px"}}>{p.tag}</span>
                      </div>
                      ...
                      <div>
                        {p.pros.map((pr,i) => (
                          <div key={i} style={{display:"flex", gap:"6px", marginBottom:"3px"}}>
                            ...
                            <span style={{fontSize:"11px", color:"#9CA3AF", lineHeight:1.45}}>{pr}</span>
                          </div>
                        ))}
                      </div>
                      <div>
                        {p.cons.map((cn,i) => (
                          <div key={i} style={{display:"flex", gap:"6px", marginBottom:"3px"}}>
                            <span style={{color:"#6B7280", fontSize:"11px", flexShrink:0, marginTop:"1px"}}>–</span>
                            <span style={{fontSize:"11px", color:"#6B7280", lineHeight:1.45}}>{cn}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <button onClick={()=>setPricingModalOpen(true)} style={{marginTop:"1.25rem", background:"transparent", border:`1px solid ${BORDER}`, color:MID, fontSize:"12px", padding:"9px 18px", borderRadius:"2px", cursor:"pointer", fontFamily:"inherit", letterSpacing:"0.04em", width:"100%"}}>
        ...
        {q.type !== "choice" && q.type !== "plan" && (
          <div style={{display:"flex", gap:"10px", marginTop:"2rem", alignItems:"center"}}>
            {step > 0 && (
              <button onClick={handleBack} style={{background:"transparent", border:`1px solid ${BORDER}`, color:MID, padding:"11px 20px", fontSize:"13px", borderRadius:"2px", cursor:"pointer", fontFamily:"inherit"}}>← Back</button>
            )}
        ...
        {(q.type === "choice" || q.type === "plan") && step > 0 && (
          <button onClick={handleBack} style={{marginTop:"1rem", background:"transparent", border:"none", color:MID, fontSize:"12px", cursor:"pointer", fontFamily:"inherit", padding:0}}>← Back</button>
        )}
    ...
    <div style={{minHeight:"80vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"4rem 2rem"}}>
      <div style={{width:"100%", maxWidth:"380px"}}>
        ...
        <div style={{marginTop:"1.5rem", display:"flex", justifyContent:"space-between"}}>
          <a href="/forgot-password" style={{fontSize:"12px", color:MID, textDecoration:"none"}}>Forgot password?</a>
