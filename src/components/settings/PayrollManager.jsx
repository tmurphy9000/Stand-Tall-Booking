        ...
        <CardContent className="p-4">
          {gustoLoading ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    {gustoConnection.company_name && (
                      <p className="text-xs text-muted-foreground">{gustoConnection.company_name}</p>
                    )}
            ...
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed border-t border-amber-100 pt-2">
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{info.full_legal_name}</p>
                  </div>
        ...
        {sensitiveInfo.length === 0 && !isGustoConnected && (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">No payroll information yet</p>
              <p className="text-xs text-muted-foreground mt-1">Invite barbers to add their banking details</p>
            </CardContent>
          </Card>
        )}
      </div>
        ...
        <div className="space-y-6 mt-2">
          ...
          <div className="border-t pt-4">
            ...
            <div className="space-y-2">
              {barbers.filter(b => b.is_active !== false).map(barber => (
                <div
                  key={barber.id}
                  className="flex items-center justify-between px-3 py-2 rounded-lg bg-muted/30 dark:bg-muted/30 border border-border dark:border-border"
                >
                  <p className="text-xs font-medium">{barber.name}</p>
                  {/* Placeholder — real sync check will be wired up later */}
                  <span className="text-[10px] font-medium bg-muted dark:bg-muted text-muted-foreground dark:text-gray-300 px-2 py-0.5 rounded-full">
                    Not yet synced
                  </span>
                </div>
              ))}
              {barbers.filter(b => b.is_active !== false).length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-3">No active barbers found.</p>
              )}
            </div>
          </div>
          ...
          <div className="border-t pt-4">
            ...
            {payrollRunsLoading ? (
              ...
              <div className="text-center py-6 rounded-lg bg-muted/30 dark:bg-muted/30 border border-border dark:border-border">
                <DollarSign className="w-6 h-6 mx-auto text-gray-300 mb-1.5" />
                <p className="text-xs text-muted-foreground">No payroll runs yet — click <span className="font-medium">Run Payroll</span> to get started.</p>
              </div>
            ) : (
              <div className="rounded-lg border border-border dark:border-border overflow-hidden">
                ...
                <table className="w-full text-xs min-w-[320px]">
                  <thead>
                    <tr className="bg-muted/30 dark:bg-muted/30 border-b border-border dark:border-border">
                      <th className="text-left px-3 py-2 font-medium text-muted-foreground">Pay Period</th>
                  ...
                  <tbody>
                    {payrollRuns.slice(0, 10).map((run, i) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-3 py-2 text-muted-foreground dark:text-gray-300">
                            {start && end
                          ...
                          <td className="px-3 py-2 text-right font-medium text-muted-foreground dark:text-gray-300">
                            {netPay != null
              ...
              return (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-lg bg-muted/30 dark:bg-muted/30 border border-border dark:border-border px-4 py-3">
                  ...
                  {payrollDetail.employee_compensations?.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-muted-foreground dark:text-gray-300 mb-2">Employee Breakdown</p>
                      <div className="rounded-lg border border-border dark:border-border overflow-hidden">
                          ...
                          <tbody>
                            {payrollDetail.employee_compensations.map((emp) => (
                              <tr key={emp.employee_uuid} className="border-b border-gray-50 dark:border-border last:border-0">
                                <td className="px-3 py-2 font-medium text-muted-foreground dark:text-gray-300">{emp.employee_name}</td>
                                <td className="px-3 py-2 text-right text-muted-foreground">{fmt(emp.gross_pay)}</td>
                                <td className="px-3 py-2 text-right text-muted-foreground dark:text-gray-300 font-medium">{fmt(emp.net_pay)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  {payrollDetail.contractor_payments?.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-muted-foreground dark:text-gray-300 mb-2">Contractor Payments</p>
                      <div className="rounded-lg border border-border dark:border-border overflow-hidden">
                        <table className="w-full text-xs">
                          <thead>
                            <tr className="bg-muted/30 dark:bg-muted/30 border-b border-border dark:border-border">
                              <th className="text-left px-3 py-2 font-medium text-muted-foreground">Name</th>
                              <th className="text-right px-3 py-2 font-medium text-muted-foreground">Wage</th>
                          ...
                          <tbody>
                            {payrollDetail.contractor_payments.map((cp) => (
                              <tr key={cp.contractor_uuid} className="border-b border-gray-50 dark:border-border last:border-0">
                                <td className="px-3 py-2 font-medium text-muted-foreground dark:text-gray-300">{cp.contractor_name}</td>
                                <td className="px-3 py-2 text-right text-muted-foreground dark:text-gray-300 font-medium">{fmt(cp.wage)}</td>
                                {payrollDetail.contractor_payments.some(c => c.hours) && (
                                  <td className="px-3 py-2 text-right text-muted-foreground">{cp.hours ?? "—"}</td>
                                )}
                              </tr>
                            ))}
                          </tbody>
                  ...
                  {payrollDetail.employee_compensations?.length === 0 &&
                    payrollDetail.contractor_payments?.length === 0 && (
                    <p className="text-xs text-muted-foreground text-center py-4">No employee breakdown available for this run.</p>
                  )}
                </>
              );
            })() : null}
